 ...
 * 创建默认配置
 */
export function createDefaultConfig(): NotifierConfig {
  return {
    enabled: true,
    logToConsole: true,
    onDependencyReady: (subFeatureId: string, readyFor: string[]) => {
...
export function isDependencyReady(
  ...
  subFeatureStates: Map<string, { status: string; phase: number }>
): boolean {
  // 获取该子特性的依赖列表
 ...
 */
export function findDependentSubFeatures(
 ...
 * @returns 通知结果
 */
export async function notifyDependencyReady(
  completedSubFeatureId: string,
  dependencies: Record<string, string[]>,
  subFeatureStates: Map<string, { status: string; phase: number }>,
  ...
  const actuallyReadySubFeatures: string[] = [];
