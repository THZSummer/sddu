...
import { 
  ...
  agentRegistry, 
  IAgentRegistry,
} from '../../../adapters/opencode/agents/registry';

import { AgentMetadata } from '../../../shared/types';  // 从统一输出导入
...
describe('AgentRegistry 测试', () => {
  ...
  test('获取所有 Agents', () => {
    const testAgents: AgentMetadata[] = [
  ...
  test('验证全局单例实例', () => {
    ...
    expect(typeof (agentRegistry as IAgentRegistry).register).toBe('function');
    expect(typeof (agentRegistry as IAgentRegistry).get).toBe('function');
    expect(typeof (agentRegistry as IAgentRegistry).has).toBe('function');
  });

  test('覆盖已存在的 Agent', () => {
    const initialAgent: AgentMetadata = {
      name: 'duplicate-test',
...
function verifyInterfaceImplementation(): IAgentRegistry {
