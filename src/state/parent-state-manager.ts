...
export class ParentStateManager {
   ...
   */
  public async scanAndUpdateParentState(parentDir: string, stateLoader: StateLoader): Promise<boolean> {
    try {
      // First, read the parent's current state
      let parentState = await stateLoader.get(parentDir) as StateV2_1_0 | null;
      ...
      const childrens: ChildFeatureInfo[] = childFeatures.map(child => ({
      ...
      const updatedParentState: StateV2_1_0 = {
        ...parentState,
        childrens: childrens,
      ...
      if (error instanceof TreeStructureError) {
        ...
        throw new TreeStructureError(
          ErrorCode.PARENT_STATE_UPDATE_FAILED,
          `Unexpected error updating parent state in ${parentDir}: ${error.message}`
        );
      }
    }
  }
    ...
    try {
      ...
      const childPromises = parentNode.children.map(async (childNode) => {
        ...
        return {
          ...
          state: childState as StateV2_1_0,
      ...
      throw new TreeStructureError(
        ...
        `Failed to discover child features in ${parentDir}: ${error.message}`
      );
    }
  }
}