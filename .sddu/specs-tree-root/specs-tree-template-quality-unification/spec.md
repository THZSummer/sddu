...
| 输出模板 | 7 | `src/templates/agents/output/sddu-*.md.hbs` |
...
| FR# | 需求 | 对应 G | 对应 5.1 | 验收标准 |
|-----|------|:------:|:--------:|----------|
| FR-013 | **骨架**：统一章节骨架，顺序为：角色定位与职责边界 → 执行顺序 → 依赖关系 → 前置验证 → 工作流程 → 输出模板 → 规则 → 异常处理 → 示例对话 → 修订记录。同功能章节统一命名。不适用于某 Agent 的章节可标注"不适用"或省略内容，但骨架位置保留 | G1, G2 | — | 逐文件章节序列一致 |
| FR-014 | **标题**：标题格式统一。基准格式：`# 🎯 SDDU [角色名称]`。类型后缀按 5.1 标注：主流程 Agent 带 `阶段 X/6`，辅助 Agent 带 `独立` 或 `触发`，入口 Agent 带 `入口` | G1, G2 | — | 逐文件首行标题一致 |
| FR-015 | **职责边界节**：统一使用四字段格式，直接承接 5.1 边界声明：`- **负责**:`、`- **输入**:`、`- **输出**:`、`- **不负责**:`。辅助 Agent（roadmap/tree/docs）的"不负责"须覆盖与其他辅助 Agent 的边界；"不负责"措辞遵循 EC-003（使用"主责"而非"禁止"，允许协作但明确主责归属） | G3 | FR-001 ~ FR-011 | 逐 Agent 检查四字段齐全，边界措辞不违反 EC-003 |
| FR-016 | **依赖关系节**：统一使用两字段格式：`- **前置条件**:`、`- **下游**:`。输入/输出已在职责边界节（FR-015）承载，不在此节重复 | G1, G2 | — | 逐节检查两字段存在 |
| FR-017 | **异常处理表格**：统一使用 `| 场景 | 处理方式 |` 两列格式，所有 Agent 包含该表格。不适用时按 FR-013 标注"不适用" | G1, G2 | — | 逐 Agent 检查表格存在且列数一致 |
| FR-018 | **修订记录**：模板末尾包含 `## 📝 修订记录` 节，每条记录含版本号、修订日期、变更摘要 | G1 | — | 逐文件检查修订记录节存在，节内至少一条修订条目 |
| FR-019 | **格式细则**：代码块统一使用 3 反引号（```）；所有 Agent 模板缩进风格统一（空格缩进、层级一致），原 sddu-review / sddu-validate 专项缩进问题一并解决 | G1, G2 | — | 无 4 反引号代码块；逐文件缩进一致 |

### 5.3 Agent 产物模板格式规范

| FR# | 需求 | 对应 G | 对应 5.1 | 验收标准 |
|-----|------|:------:|:--------:|----------|
| FR-020 | **文档格式模板**：产物模板为完整文档格式骨架，定义 Agent 产出文档的完整章节结构（如 spec 模板含元数据/上下文/目标/用户故事/FR/NFR/EC/开放问题/修订记录），由 Agent 将工作流产出按语义自行匹配填充到模板对应区域。每个模板末尾均含修订记录表 | G1, G2 | — | 逐文件章节结构完整，内容可独立作为文档格式参考 |
| FR-021 | **格式一致性**：各产物模板的章节命名和层级风格统一，表格格式一致，缩进使用空格。内容对齐 5.1 各 Agent 的「输出」字段（如 spec→需求规范含 FR/NFR/EC、plan→技术方案含方案对比/ADR/文件影响）。辅助 Agent（roadmap/tree/docs）按 EC-004 标注输出为内置固定格式，不走用户自定义模板覆盖路径 | G1, G2 | FR-001~FR-011 | 逐文件格式一致，表格列数/缩进/代码块风格统一 |
| FR-022 | **完成报告归属**：完成报告（含自动触发 `@sddu-tree` 文档更新）归属 Agent 指令模板的「完成协议」节，不置于产物模板中。产物模板专注于文档格式定义——完成报告是 Agent 执行协议，非输出文档格式 | G2 | — | 指令模板逐文件「完成协议」节存在且含自动触发块；产物模板无完成报告残留 |
...
| 输出模板 | `src/templates/agents/output/sddu-*.md.hbs`（7 个） | 完整文档格式骨架，由各 Agent 按语义自行匹配填充 |
