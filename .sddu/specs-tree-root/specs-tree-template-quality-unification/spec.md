...
| FR# | 需求 | 对应 G | 对应 5.1 | 验收标准 |
|-----|------|:------:|:--------:|----------|
| FR-020 | **骨架**：统一章节骨架，顺序为：标题 → 输出格式 → 完成报告。不适用于某 Agent 的章节可标注"不适用"，但骨架位置保留 | G1, G2 | — | 逐文件章节序列一致 |
| FR-021 | **输出格式节**：节名 `## 输出格式`，定义该 Agent 产物的格式模板。内容对齐 5.1 各 Agent 的「输出」字段（如 spec→需求规范、plan→技术方案）。辅助 Agent（roadmap/tree/docs）按 EC-004 标注输出为内置固定格式 | G1, G2 | FR-001~FR-011 | 逐文件节名一致，内容可追溯到 5.1 输出字段 |
| FR-022 | **完成报告节**：节名 `## 完成报告`，节内固定包含独立的 `### 自动触发文档更新` 子节，不可内嵌于其他段落 | G1, G2 | — | 逐文件完成报告节存在；自动触发块为独立 `###` 小节 |
