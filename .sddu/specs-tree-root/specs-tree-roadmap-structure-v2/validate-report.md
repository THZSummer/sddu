...
| 模板篇幅 | ≤170 | 169 | ✅ |
| 产物篇幅 | ≤600（R1 起，原 450） | 全量迁移 576（≤600）；抽样口径 432 | ✅ |
...
## 7. R1 回归段（post-validation / v3.2.1）
> 对 §5 遗留改进 1 的修复回归（表格类 preserve 区行自然键合并）

| 断言 | 方法 | 结果 |
|------|------|------|
| §4 `issue-list` 表格渲染 | 修复后模板 `<<>>`→token 替换，取 `issue-list` zone 正文，marked（GFM）渲染 | 含完整 `<table>`（1 表），表头 + 数据 2 行，数据行位于 `<td>`；`<p>|…|</p>` 退化 = **false**；无 HTML 注释残留 |
| §8 `revision-log` 表格渲染 | 同上，取 `revision-log` zone 正文 | 同上（1 表 / 2 行 / 数据入 `<td>` / 退化 false） |
| 双渲染器交叉验证 | remark-gfm AST | 两区各 1 个 `table` 节点、各 2 行（表头 + 数据） |
| entry 注释计数 | `grep -c 'sddu:entry'` | **6 行** = 仅 §5/§6/§7 三对（块级，安全）；§4/§8 已清零 |
| 结构不回归 | `grep` / `wc -l` | H2=8、zone 9/9、mermaid=8 不变；模板 **165 ≤ 170**；构建 `dist ≡ src` |

**强度标注**：§4/§8 表格渲染为 **实测**（node v24.15.0 + marked GFM + remark-gfm 双实现，脚本 `/tmp/opencode/md-render/check.mjs`，断言 PASS）；因环境无 `mmdc`，Mermaid 仍为静态核查。等效静态断言亦成立：两表内无任何 `<!--` 行，表头与数据行连续无空行 / 注释隔断。

**结论**：review / validate 共同的唯一遗留缺陷（表格 entry 注释截断）已消除，验证结论维持 ✅ 通过；产物预算按 NFR-002（v1.1）升为 600，全量迁移 576 直接合规。
...
| v1.1 | 追加 §7 R1 回归段：v3.2.1 修复后 §4/§8 表格渲染双渲染器实测 PASS（完整 `<table>`、数据入 `<td>`）；entry 注释 6 行（仅块级 §5/§6/§7）；NFR-002 预算 450→600 | 2026-09-24 | SDDU Team |
