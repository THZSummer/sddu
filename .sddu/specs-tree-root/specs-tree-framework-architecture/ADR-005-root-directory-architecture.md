...
│   (subdirectories)     各功能域（源码、E2E、脚本、文档、示例）统一在独立子目录。单元/集成测试统一在 src/__tests__/ 内，E2E 独立为 e2e/（见 ADR-004）
│
├── 构建产出层         ← dist/
│   (gitignored)
│
...
6. **测试分层就近 + E2E 独立**：单元/集成/回归测试统一在 `src/__tests__/` 内——维护者在 `src/` 目录内即可完成编码→单测→集成测试的完整 TDD 环路。E2E 独立为 `e2e/` 顶层目录（自带 jest 配置、不收集覆盖率、独立生命周期），与单元/集成测试物理隔离，CI 编排更清晰
