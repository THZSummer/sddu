...
├── build.md                               # 构建报告 — 完成 12/12 任务，~80 新增/~50 修改/~30 删除文件
├── review.md                              # 审查报告 — 静态分析 ~85 文件，19 通过/2 改进建议/0 阻塞
├── validate.md                            # 验证报告 — 动手验证：类型检查、import 规则、核心测试、构建验证、漂移扫描
...
| tasks.md | 任务分解：12 个原子任务（TASK-001~012）/ 5 个执行波次 — Wave 1 根目录脚本收敛 + shared/ 共享层，Wave 2~5 业务域渐进迁移 | ✅ validated |
| tasks.json | 任务 JSON：12 tasks 结构化 — id / complexity (S/M/L) / wave / dependencies / files (op) / acceptance / verify | ✅ validated |
| build.md | 构建报告：SDDU 框架源码架构重组 — 完成 12/12 任务/5 个波次，~80 新增/~50 修改/~30 删除文件 | ✅ validated |
| review.md | 审查报告：SDDU 框架源码架构重组 — 静态分析 ~85 文件（47 .ts + 38 .test.ts），19 通过/2 改进建议/0 阻塞 | ✅ validated |
| validate.md | 验证报告：SDDU 框架源码架构重组 — 动手验证（类型检查、import 规则、核心测试、构建验证、漂移扫描） | ✅ validated |
...
| state.json | 状态文件 — Feature ID: FR-FRAMEWORK-ARCH-001 | validated (✅ completed) |
