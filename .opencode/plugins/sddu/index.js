...
import { 
// Discovery 相关类型和类
DiscoveryWorkflowEngine, CoachingLevel, DiscoveryStateValidator, DISCOVERY_WORKFLOW, } from './discovery/workflow-engine';
// 重新导入状态相关类型和类
import { 
// 状态管理相关类和接口
StateMachine, DependencyChecker, StateLoader, validateState, } from './state/machine';
...
import { 
// 迁移相关类和类型
migrateState, } from './state/migrator';
// 从命令模块导入
...
export const SDDUPlugin = async ({ project, client, $, directory, worktree }) => {
    ...
    // Map status to valid transition target names used by StateMachine
    const statusToTransitionTarget = {
        'specified': 'specified',
        'planned': 'planned',
        'tasked': 'tasked',
    ...
    return {
        // Register sddu_update_state tool for Agent workflow
        tool: {
            sddu_update_state: tool({
                description: 'Update the state of a SDDU feature. Transitions the feature through the workflow: specified -> planned -> tasked -> building -> reviewed -> validated. Automatically handles phase, phaseHistory, and validation.',
                args: {
                    feature: tool.schema.string().describe('Feature path/identifier (e.g., "my-feature" or "specs-tree-parent/specs-tree-child")'),
                    status: tool.schema.string().describe('Target status: specified, planned, tasked, building, reviewed, validated (or aliases: discovered, implementing, completed)'),
                    comment: tool.schema.string().optional().describe('Optional comment explaining the state change'),
                    data: tool.schema.object({}).passthrough().optional().describe('Optional additional data to attach to the state'),
                },
                async execute(args, context) {
                    const { feature, status: rawStatus, comment, data } = args;
                    if (!feature || !rawStatus) {
                        return JSON.stringify({ success: false, error: 'Missing required parameters: feature and status' });
                    }
                    // Normalize status: accept both 'status' and legacy 'state' field
                    const inputStatus = rawStatus.toLowerCase().trim();
                    // Map alias to valid schema status
                    const mappedStatus = statusAliasMap[inputStatus];
                    if (!mappedStatus) {
                        const validStatuses = Object.keys(statusAliasMap).join(', ');
                        return JSON.stringify({
                            success: false,
                            error: `Invalid status: "${inputStatus}". Valid statuses: ${validStatuses}`,
                        });
                    }
                    try {
                        const transitionTarget = statusToTransitionTarget[mappedStatus];
                        const result = await stateMachine.updateState(
                            feature,
                            transitionTarget,
                            data || {},
                            context.agent || 'sddu_update_state',
                            comment || `State updated to ${mappedStatus} via tool`
                        );
                        return JSON.stringify({
                            success: true,
                            ...
                            message: `Feature "${feature}" updated to status: ${mappedStatus} (phase ${result.phase})`,
                        });
                    } catch (error) {
                        return JSON.stringify({
                            success: false,
                            error: error.message,
                            hint: 'Ensure the feature exists and the state transition is valid. Use "force: true" to skip validation if needed.',
                        });
                    }
                },
            }),
...
export { 
// Discovery
DISCOVERY_WORKFLOW, DiscoveryWorkflowEngine, CoachingLevel, DiscoveryStateValidator, 
// State Management
AutoUpdater, StateMachine, DependencyChecker, StateLoader, // New export for distributed state
ParentStateManager, // New export for parent feature management
TreeStateValidator, validateState, 
// Migration  
SdduMigrateSchemaCommand, migrateState, };
